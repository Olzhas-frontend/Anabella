...
const html = {
		...
		src_all: ['templates/template/*.pug', 'templates/parts/*.pug', 'templates/*.pug'],
		dest: projectPath,
	},
	css = {
		...
		file_name: 'front.css',
		src_all: ['sass/*.sass', 'sass/media/*.sass', 'sass/grid/*.sass', 'sass/ui/*.sass'],
		dest: projectPath,
	},
	js = {
		src_build: ['js/*.js'],
...
gulp.task('move_fonts', function () {
	return gulp
		.src('sass/fonts/*.*')
		.pipe(flatten({ includeParents: 0 }))
		.pipe(gulp.dest(projectPath + '/css/fonts'));
});
...
gulp.task('min_js', function () {
	return gulp.src(js['src_all']).pipe(minifyjs()).pipe(gulp.dest(js['dest']));
});
...
gulp.task('min_main', function () {
	return gulp.src(img['src_all']).pipe(imagemin()).pipe(gulp.dest(img['dest']));
});
...
gulp.task(
	'default',
	gulp.parallel('watch', 'html', 'js', 'move_fonts', 'css', 'browserSync', 'move_images', 'media_query')
);
